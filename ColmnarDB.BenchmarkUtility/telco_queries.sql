...
SELECT ageId FROM TargetLoc1B WHERE ROUND(POWER(latitude, 2)) > MAX((longitude * PI()));
SELECT hwOsId, AVG(ageId) FROM TargetLoc1B WHERE genderId != -1 GROUP BY hwOsId;
SELECT genderId, COUNT(genderId) FROM TargetLoc1B WHERE ROUND(POWER(SQRT(longitude), 2)) == ROUND(longitude) GROUP BY genderId;
SELECT genderId, COUNT(genderId) FROM TargetLoc1B WHERE ROUND(POWER(SQRT(longitude), 2)) == ROUND(SQUARE(ROOT(longitude, 2))) GROUP BY genderId;