...
#include "../qikkDB/TCPClientHandler.h"
...
#include "../qikkDB/Types/Point.pb.h"
...
TEST(ClientHandlerTests, TestHandlerInfo)
{
	...
	infoMessage.set_code(QikkDB::NetworkClient::Message::InfoMessage_StatusCode_CONN_END);
...
TEST(ClientHandlerTests, TestHandlerCSV)
{
	...
	IClientHandler* handlerPtr = handler.get();
	ClientPoolWorker tempWorker(std::move(handler), boost::asio::ip::tcp::socket(context), 60000);
	QikkDB::NetworkClient::Message::CSVImportMessage csvMessage;
...
TEST(ClientHandlerTests, TestHandlerCSVWithTypes)
{
	...
	csvMessage.set_databasename("testCSV");
	csvMessage.set_payload("test1,test2,test3\n1,2,3");
	csvMessage.add_columntypes(QikkDB::NetworkClient::Message::COLUMN_INT);
	csvMessage.add_columntypes(QikkDB::NetworkClient::Message::COLUMN_INT);
	csvMessage.add_columntypes(QikkDB::NetworkClient::Message::COLUMN_INT);
	handlerPtr->HandleCSVImport(tempWorker, csvMessage);
	auto db = Database::GetDatabaseByName("testCSV");
...
TEST(ClientHandlerTests, TestHandlerQuery)
{
	...
	ASSERT_NE(dynamic_cast<QikkDB::NetworkClient::Message::InfoMessage*>(responsePtr.get()), static_cast<QikkDB::NetworkClient::Message::InfoMessage*>(nullptr));
	ASSERT_EQ(dynamic_cast<QikkDB::NetworkClient::Message::InfoMessage*>(responsePtr.get())->code(), QikkDB::NetworkClient::Message::InfoMessage_StatusCode_WAIT);
	QikkDB::NetworkClient::Message::InfoMessage getNextResultMessage;
	getNextResultMessage.set_code(QikkDB::NetworkClient::Message::InfoMessage_StatusCode_GET_NEXT_RESULT);
	getNextResultMessage.set_message("");
	auto queryResponsePtr = handlerPtr->HandleInfoMessage(tempWorker, getNextResultMessage);
	ASSERT_NE(dynamic_cast<QikkDB::NetworkClient::Message::QueryResponseMessage*>(queryResponsePtr.get()), static_cast<QikkDB::NetworkClient::Message::QueryResponseMessage*>(nullptr));
	auto& payload = dynamic_cast<QikkDB::NetworkClient::Message::QueryResponseMessage*>(queryResponsePtr.get())->payloads().at("test.test").intpayload().intdata();
	auto nullMaskPayload = dynamic_cast<QikkDB::NetworkClient::Message::QueryResponseMessage*>(queryResponsePtr.get())->nullbitmasks().at("test.test").nullmask();
...
TEST(ClientHandlerTests, TestHandlerBulkImportMultiSplitColumn)
{
	...
	int32_t dataBuff[] = {1,2,3,4,5};
	handlerPtr->HandleBulkImport(tempWorker,bulkImportMessage,reinterpret_cast<char*>(dataBuff));
	QikkDB::NetworkClient::Message::BulkImportMessage bulkImportMessage2;
	bulkImportMessage2.set_tablename("test");
	bulkImportMessage2.set_columnname("test2");
	bulkImportMessage2.set_columntype(static_cast<QikkDB::NetworkClient::Message::DataType>(DataType::COLUMN_INT));
...
TEST(ClientHandlerTests, TestHandlerBulkImportProtobufColumn)
{
	...
	bulkImportMessage.set_tablename("test");
	bulkImportMessage.set_columnname("test");
	bulkImportMessage.set_columntype(static_cast<QikkDB::NetworkClient::Message::DataType>(DataType::COLUMN_POINT));
	bulkImportMessage.set_elemcount(5);
	QikkDB::Types::Point aPoint;
	...
	auto& column1 = db->GetTables().at("test").GetColumns().at("test");
	ASSERT_EQ(aPoint.geopoint().latitude(), dynamic_cast<BlockBase<QikkDB::Types::Point>&>(*dynamic_cast<ColumnBase<QikkDB::Types::Point>&>(*column1).GetBlocksList()[0]).GetData()[0].geopoint().latitude());
	ASSERT_EQ(aPoint.geopoint().longitude(), dynamic_cast<BlockBase<QikkDB::Types::Point>&>(*dynamic_cast<ColumnBase<QikkDB::Types::Point>&>(*column1).GetBlocksList()[0]).GetData()[0].geopoint().longitude());
	Database::RemoveFromInMemoryDatabaseList("test");
}