#include "../qikkDB/QueryEngine/GPUCore/GPUJoin.cuh"
#include "../qikkDB/QueryEngine/GPUCore/GPUFilterConditions.cuh"
#include "../qikkDB/QueryEngine/GPUCore/GPUMemory.cuh"
#include "../qikkDB/QueryEngine/CPUJoinReorderer.cuh"
...
TEST(GPUJoinTests, JoinTest)
{
    ...
    */
    ...
    bool aborted = false;
    GPUJoin::JoinTableRonS<FilterConditions::equal>(resultColumnQAJoinIdx, resultColumnQBJoinIdx,
                                                    ColumnR_, ColumnS_, BLOCK_SIZE, aborted);
...
TEST(GPUJoinTests, ReorderCPUTest)
{
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////
    // Reorder the blocks
    cuda_ptr<int32_t> d_outRBlock(BLOCK_SIZE);
    cuda_ptr<int32_t> d_outSBlock(BLOCK_SIZE);
    ...
    {
        CPUJoinReorderer::reorderByJIPushToGPU(d_outRBlock.get(), outRBlockDataElementCount,
                                               ColumnR_, i, resultColumnQAJoinIdx, BLOCK_SIZE);
        CPUJoinReorderer::reorderByJIPushToGPU(d_outSBlock.get(), outSBlockDataElementCount,
                                               ColumnS_, i, resultColumnQBJoinIdx, BLOCK_SIZE);

        // Test if the results are equal
        GPUMemory::copyDeviceToHost(outRBlock.data(), d_outRBlock.get(), outRBlockDataElementCount);
        GPUMemory::copyDeviceToHost(outSBlock.data(), d_outSBlock.get(), outSBlockDataElementCount);
