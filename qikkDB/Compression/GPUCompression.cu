...
template <typename T>
bool compressAAFL(const int CWARP_SIZE,
                  T* const hostUncompressed,
                  ...
                  T maxValue)
{
    ...
    {
        if (std::numeric_limits<T>::max() - maxValue < -minValue) offset = 0;
    }

    int64_t uncompressedDataSize = uncompressedElementsCount * sizeof(T); // size in bytes
    int64_t compressionBlocksCount =
        (uncompressedDataSize + (sizeof(T) * CWARP_SIZE) - 1) / (sizeof(T) * CWARP_SIZE);
    ...
    container_aafl<T> cdata = {deviceCompressed,
                               static_cast<unsigned long>(uncompressedElementsCount),
                               deviceBitLength,
                               devicePositionId,
                               deviceCompressedElementsCount,
                               offset};
    ...
    if (compressedElementsCount < uncompressedElementsCount)
    {
        ...
        int positionCodedDataBitLength =
        ...
        std::move(reinterpret_cast<char*>(codedSizes),
                  reinterpret_cast<char*>(codedSizes) + (sizeof(int64_t) * 3),
                  reinterpret_cast<char*>(data.get()));
        cudaMemcpy(reinterpret_cast<char*>(data.get()) + positionCodedDataPositionId, devicePositionId,
                   compressionBlocksCount * sizeof(unsigned long), cudaMemcpyDeviceToHost);
        cudaMemcpy(reinterpret_cast<char*>(data.get()) + positionCodedDataBitLength, deviceBitLength,
                   compressionBlocksCount * sizeof(unsigned char), cudaMemcpyDeviceToHost);
        cudaMemcpy(reinterpret_cast<char*>(data.get()) + positionHostOut, deviceCompressed,
                   compressedDataSize, cudaMemcpyDeviceToHost);
        CheckCudaError(cudaGetLastError());
...
template <>
bool CompressionGPU::compressDataAAFL<int32_t>(int32_t* const hostUncompressed,
                                               int64_t uncompressedElementsCount,
                                               std::vector<int32_t>& hostCompressed,
                                               int64_t& compressedElementsCount,
                                               int32_t minValue,
                                               int32_t maxValue)
{
    return compressAAFL(32, hostUncompressed, uncompressedElementsCount, hostCompressed,
                        compressedElementsCount, minValue, maxValue);
}

template <>
bool CompressionGPU::compressDataAAFL<int64_t>(int64_t* const hostUncompressed,
                                               int64_t uncompressedElementsCount,
                                               std::vector<int64_t>& hostCompressed,
                                               int64_t& compressedElementsCount,
                                               int64_t minValue,
                                               int64_t maxValue)
{
...
template <>
bool CompressionGPU::compressDataAAFL<int8_t>(int8_t* const hostUncompressed,
                                              int64_t uncompressedElementsCount,
                                              std::vector<int8_t>& hostCompressed,
                                              int64_t& compressedElementsCount,
                                              int8_t minValue,
                                              int8_t maxValue)
{
...
template <>
bool CompressionGPU::compressDataAAFL<double>(double* const hostUncompressed,
                                              int64_t uncompressedElementsCount,
                                              std::vector<double>& hostCompressed,
                                              int64_t& compressedElementsCount,
                                              double minValue,
                                              double maxValue)
{
...
template <>
bool CompressionGPU::compressDataAAFL<float>(float* const hostUncompressed,
                                             int64_t uncompressedElementsCount,
                                             std::vector<float>& hostCompressed,
                                             int64_t& compressedElementsCount,
                                             float minValue,
                                             float maxValue)
{
    if ((minValue >= 0 && maxValue >= 0) || (minValue <= 0 && maxValue <= 0))
    {
        ...
        if (min_int32 > max_int32) std::swap(min_int32, max_int32);
        bool compressed = compressAAFL(32, host_uncompressed_int32, uncompressedElementsCount,
                                       host_compressed_int32, compressedElementsCount, min_int32, max_int32);
        if (compressed)
        {
...
template <>
bool CompressionGPU::compressDataAAFL<std::string>(std::string* const hostUncompressed,
                                                   int64_t uncompressedElementsCount,
                                                   std::vector<std::string>& hostCompressed,
                                                   int64_t& compressedElementsCount,
                                                   std::string minValue,
                                                   std::string maxValue)
{
...
template <typename T>
bool decompressAAFL(const int CWARP_SIZE,
                    T* const hostCompressed,
                    int64_t compressedElementsCount,
                    ...
                    T maxValue)
{
    ...
    unsigned long* hostPositionId =
        reinterpret_cast<unsigned long*>(reinterpret_cast<char*>(hostCompressed) + positionCodedDataPositionId);
    unsigned char* hostBitLength = reinterpret_cast<unsigned char*>(hostCompressed) + positionCodedDataBitLength;
    T* hostCompressedValuesData = reinterpret_cast<T*>(
        reinterpret_cast<char*>(hostCompressed) + positionHostOut); // data of values only without meta data

    // Copy data CPU->GPU
    cudaMemcpy(deviceCompressed, hostCompressedValuesData, compressedDataSize - (positionHostOut),
               cudaMemcpyHostToDevice); // from compression size we need to subtract leading bytes with meta info
    cudaMemcpy(devicePositionId, hostPositionId, compressionBlocksCount * sizeof(unsigned long),
...
template <>
bool CompressionGPU::decompressDataAAFL<int32_t>(int32_t* const hostCompressed,
                                                 int64_t compressedElementsCount,
                                                 std::vector<int32_t>& hostUncompressed,
                                                 int64_t& uncompressedElementsCount,
                                                 int32_t minValue,
                                                 int32_t maxValue)
{
    return decompressAAFL(32, hostCompressed, compressedElementsCount, hostUncompressed,
                          uncompressedElementsCount, minValue, maxValue);
}

template <>
bool CompressionGPU::decompressDataAAFL<int64_t>(int64_t* const hostCompressed,
                                                 int64_t compressedElementsCount,
                                                 std::vector<int64_t>& hostUncompressed,
                                                 int64_t& uncompressedElementsCount,
                                                 int64_t minValue,
                                                 int64_t maxValue)
{
...
template <>
bool CompressionGPU::decompressDataAAFL<int8_t>(int8_t* const hostCompressed,
                                                int64_t compressedElementsCount,
                                                std::vector<int8_t>& hostUncompressed,
                                                int64_t& uncompressedElementsCount,
                                                int8_t minValue,
                                                int8_t maxValue)
{
...
template <>
bool CompressionGPU::decompressDataAAFL<float>(float* const hostCompressed,
                                               int64_t compressedElementsCount,
                                               std::vector<float>& hostUncompressed,
                                               int64_t& uncompressedElementsCount,
                                               float minValue,
                                               float maxValue)
{
    if ((minValue > 0 && maxValue > 0) || (minValue < 0 && maxValue < 0))
    {
        ...
        int32_t max_int32 = *(reinterpret_cast<int32_t*>(&maxValue));
        if (min_int32 > max_int32) std::swap(min_int32, max_int32);
        bool decompressed = decompressAAFL(32, host_compressed_int32, compressedElementsCount, host_uncompressed_int32,
                                           uncompressedElementsCount, min_int32, max_int32);
        if (decompressed)
        {
...
template <>
bool CompressionGPU::decompressDataAAFL<double>(double* const hostCompressed,
                                                int64_t compressedElementsCount,
                                                std::vector<double>& hostUncompressed,
                                                int64_t& uncompressedElementsCount,
                                                double minValue,
                                                double maxValue)
{
...
template <typename T>
bool decompressAAFLOnDevice(const int CWARP_SIZE,
                            T* const deviceCompressed,
                            int64_t uncompressedElementsCount,
                            int64_t compressedElementsCount,
                            int64_t compressionBlocksCount,
                            ...
                            T maxValue)
{
    ...
    unsigned long* devicePositionId = reinterpret_cast<unsigned long*>(
        reinterpret_cast<char*>(deviceCompressed) + positionCodedDataPositionId);
    unsigned char* deviceBitLength = reinterpret_cast<unsigned char*>(deviceCompressed) + positionCodedDataBitLength;
    T* deviceCompressedValuesData = reinterpret_cast<T*>(
        reinterpret_cast<char*>(deviceCompressed) + positionDeviceOut); // data of values only without meta data

    // Decompression
    container_uncompressed<T> udata = {deviceUncompressed, static_cast<unsigned long>(uncompressedElementsCount)};
    container_aafl<T> cdata = {deviceCompressedValuesData,
                               static_cast<unsigned long>(uncompressedElementsCount),
                               deviceBitLength,
                               devicePositionId,
                               NULL,
...
template <>
bool CompressionGPU::decompressDataAAFLOnDevice<int32_t>(int32_t* const device_compressed,
                                                         int64_t data_size,
                                                         int64_t compression_data_size,
                                                         int64_t compression_blocks_count,
                                                         int32_t* const device_uncompressed,
                                                         int32_t minValue,
                                                         int32_t maxValue)
{
    return decompressAAFLOnDevice(32, device_compressed, data_size, compression_data_size,
                                  compression_blocks_count, device_uncompressed, minValue, maxValue);
}

template <>
bool CompressionGPU::decompressDataAAFLOnDevice<int64_t>(int64_t* const device_compressed,
                                                         int64_t data_size,
                                                         int64_t compression_data_size,
                                                         int64_t compression_blocks_count,
                                                         int64_t* const device_uncompressed,
                                                         int64_t minValue,
                                                         int64_t maxValue)
{
    return decompressAAFLOnDevice(32, device_compressed, data_size, compression_data_size,
...
template <>
bool CompressionGPU::decompressDataAAFLOnDevice<int8_t>(int8_t* const device_compressed,
                                                        int64_t data_size,
                                                        int64_t compression_data_size,
                                                        int64_t compression_blocks_count,
                                                        int8_t* const device_uncompressed,
                                                        int8_t minValue,
                                                        int8_t maxValue)
{
    return decompressAAFLOnDevice(32, device_compressed, data_size, compression_data_size,
...
template <>
bool CompressionGPU::decompressDataAAFLOnDevice<float>(float* const device_compressed,
                                                       int64_t data_size,
                                                       int64_t compression_data_size,
                                                       int64_t compression_blocks_count,
                                                       float* const device_uncompressed,
                                                       float minValue,
                                                       float maxValue)
{
    if ((minValue > 0 && maxValue > 0) || (minValue < 0 && maxValue < 0))
    {
        ...
        bool compressed = decompressAAFLOnDevice(32, device_compressed_int32, data_size,
                                                 compression_data_size, compression_blocks_count,
                                                 device_uncompressed_int32, min_int32, max_int32);
        return compressed;
    }
    return false;
}
...
template <>
bool CompressionGPU::decompressDataAAFLOnDevice<double>(double* const device_compressed,
                                                        int64_t data_size,
                                                        int64_t compression_data_size,
                                                        int64_t compression_blocks_count,
                                                        double* const device_uncompressed,
                                                        double minValue,
                                                        double maxValue)
{
    return false;
