...
public:
    ...
    CSVDataImporter(const char* inputString,
                    const char* tableName,
                    bool header = true,
                    char delimiter = ',',
                    char quotes = '\'',
                    char decimal = '.')
    : input_(inputString), inputSize_(strlen(inputString)), tableName_(tableName), header_(header),
    ...
    void ExtractHeaders();
    void ExtractTypes();
    void SetTypes(const std::vector<std::pair<DataType, DataTypeExternal>>& types);
...
private:
    ...
    std::vector<std::pair<DataType, DataTypeExternal>> dataTypes_;
    int numThreads_ = 1;
    std::mutex insertMutex_;

    std::pair<DataType, DataTypeExternal> IdentifyDataType(std::vector<std::string> columnValues);
    void ParseAndImport(int threadId,
                        int32_t blockSize,
                        Table& table);
};
