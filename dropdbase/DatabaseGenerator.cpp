...
/// If there are more columns of the same type, each column will have a different data according to
/// formula: k % (1024 * typeColumnCount).
/// </summary>
/// <param name="databaseName">Name of the database. There is no default value, must be
/// specified.</param> <param name="blockCount">Number of blocks of data that will be in each
/// column. Default value is set to 2.</param> <param name="blockSize">Length of each block of data.
/// Default value is set to 2^18.</param> <param name="sameDataInBlocks">If set to true, all rows
...
{
    return DatabaseGenerator::GenerateDatabase(databaseName, blockCount, blockSize,
                                               sameDataInBlocks, std::nullopt, std::nullopt);
}
...
DatabaseGenerator::GenerateDatabase(const char* databaseName,
                                    int blockCount,
                                    int blockSize,
                                    bool sameDataInBlocks,
                                    std::optional<const std::vector<std::string>> tablesNames,
                                    std::optional<const std::vector<DataType>> columnsTypes)
{
    ...
    // column counts - used when there are more columns of the same type, then this is used for
    // making data in those columns different NOTE: There has to be block size big enough, so that
    // the change in data is visible, because algorithm is: k % (1024 * intColumnCount)
    ...
    for (const auto& tableName : tableNames)
    {
        ...
        for (const auto& columnType : columnTypes)
        {
            ...
            {
                integerColumnCount++;
                table.CreateColumn((std::string("colInteger") + std::to_string(integerColumnCount)).c_str(),
                                   COLUMN_INT);
                auto& columns = table.GetColumns();
                auto& column = dynamic_cast<ColumnBase<int32_t>&>(*columns.at(
                    (std::string("colInteger") + std::to_string(integerColumnCount)).c_str()));
                ...
                for (int i = 0; i < blockCount; i++)
                {
                    ...
                    for (int k = 0; k < blockSize; k++)
                    {
                        if (k % 2)
                        {
                            integerData.push_back(sameDataInBlocks ?
                                                      1 :
                                                      ((k % (1024 * integerColumnCount)) +
                                                       (integerColumnCount >= 3 ? 200 * integerColumnCount : 0)));
                        }
                        else
                        {
                            integerData.push_back(sameDataInBlocks ?
                                                      -1 :
                                                      (((k % (1024 * integerColumnCount)) * -1) -
                                                       (integerColumnCount >= 3 ? 200 * integerColumnCount : 0)));
                        }
                    }
            ...
            case COLUMN_LONG:
            {
                ...
                auto& column = dynamic_cast<ColumnBase<int64_t>&>(
                    *columns.at((std::string("colLong") + std::to_string(longColumnCount)).c_str()));
                ...
                for (int i = 0; i < blockCount; i++)
                {
                    ...
                    for (int k = 0; k < blockSize; k++)
                    {
                        ...
                        {
                            int64_t result = (static_cast<int64_t>(2 * pow(10, k % 19))) +
                                             k % (1024 * longColumnCount);
                            ...
                            {
                                longData.push_back(sameDataInBlocks ?
                                                       static_cast<int64_t>(pow(10, k % 19)) * -1 :
                                                       result * (-1));
                            }
                        }
            ...
            case COLUMN_FLOAT:
            {
                ...
                auto& columns = table.GetColumns();
                auto& column = dynamic_cast<ColumnBase<float>&>(
                    *columns.at((std::string("colFloat") + std::to_string(floatColumnCount)).c_str()));
                ...
                for (int i = 0; i < blockCount; i++)
                {
                    ...
                    for (int k = 0; k < blockSize; k++)
                    {
                        if (k % 2)
                        {
                            floatData.push_back(sameDataInBlocks ?
                                                    (float)0.1111 :
                                                    (float)(k % (1024 * floatColumnCount) + 0.1111));
                        }
                        else
                        {
                            floatData.push_back(sameDataInBlocks ?
                                                    (float)-0.1111 :
                                                    (float)((k % (1024 * floatColumnCount) + 0.1111) * -1));
                        }
                    }
            ...
            {
                doubleColumnCount++;
                table.CreateColumn((std::string("colDouble") + std::to_string(doubleColumnCount)).c_str(),
                                   COLUMN_DOUBLE);
                auto& columns = table.GetColumns();
                auto& column = dynamic_cast<ColumnBase<double>&>(
                    *columns.at((std::string("colDouble") + std::to_string(doubleColumnCount)).c_str()));
                ...
                for (int i = 0; i < blockCount; i++)
                {
                    ...
                    for (int k = 0; k < blockSize; k++)
                    {
                        if (k % 2)
                        {
                            doubleData.push_back(sameDataInBlocks ? 0.1111111 :
                                                                    k % (1024 * doubleColumnCount) + 0.1111111);
                        }
                        else
                        {
                            doubleData.push_back(sameDataInBlocks ?
                                                     -0.1111111 :
                                                     (k % (1024 * doubleColumnCount) + 0.1111111) * -1);
                        }
                    }
            ...
            case COLUMN_POINT:
            {
                ...
                for (int i = 0; i < blockCount; i++)
                {
                    ...
                    while (k + 4 < blockSize)
                    {
                        pointData.push_back(
                            sameDataInBlocks ?
                                PointFactory::FromWkt("POINT(15 15.4)") :
                                PointFactory::FromWkt(
                                    std::string("POINT(") +
                                    std::to_string(15 + 5 * (pointColumnCount - 1) + k * 0.001) + " " +
                                    std::to_string(15.4 + 5 * (pointColumnCount - 1) + k * 0.001) + ")")); // outside
                        pointData.push_back(
                            sameDataInBlocks ?
                                PointFactory::FromWkt("POINT(15.5 10.5)") :
                                PointFactory::FromWkt(
                                    std::string("POINT(") +
                                    std::to_string(15.5 + 5 * (pointColumnCount - 1) + k * 0.001) + " " +
                                    std::to_string(10.5 + 5 * (pointColumnCount - 1) + k * 0.001) + ")")); // on line, so it is inside
                        pointData.push_back(
                            sameDataInBlocks ?
                                PointFactory::FromWkt("POINT(10.5 10.5)") :
                                PointFactory::FromWkt(
                                    std::string("POINT(") +
                                    std::to_string(10.5 + 5 * (pointColumnCount - 1) + k * 0.001) + " " +
                                    std::to_string(10.5 + 5 * (pointColumnCount - 1) + k * 0.001) + ")")); // same vertex, so it is inside
                        pointData.push_back(
                            sameDataInBlocks ?
                                PointFactory::FromWkt("POINT(-30 -30.6)") :
                                PointFactory::FromWkt(
                                    std::string("POINT(") +
                                    std::to_string((30 + 5 * (pointColumnCount - 1) + k * 0.001) * -1) + " " +
                    ...
                    while (k < blockSize)
                    {
                        pointData.push_back(
                            sameDataInBlocks ?
                                ...
                                PointFactory::FromWkt(
                                    std::string("POINT(") +
                                    std::to_string(15 + 5 * pointColumnCount + k * 0.001) + " " +
            ...
            case COLUMN_POLYGON:
            {
                ...
                for (int i = 0; i < blockCount; i++)
                {
                    ...
                    for (int k = 0; k < blockSize; k++)
                    {
                        polygonData.push_back(
                            sameDataInBlocks ?
                                ComplexPolygonFactory::FromWkt(
                                    "POLYGON((2 2, 4 2, 2 2),(5 15.5, 10.5 10.5, 20.5 10.5, 25 "
                                    "15.5, 20.5 20, 10.5 20, 5 15.5))") :
                                ComplexPolygonFactory::FromWkt(
                                    std::string("POLYGON((2 2, 4 2, 2 2),(") +
                                    std::to_string(5 + 5 * (polygonColumnCount - 1) + k * 0.001) + " " +
                                    std::to_string(15.5 + 5 * (polygonColumnCount - 1) + k * 0.001) + ", " +
                                    std::to_string(10.5 + 5 * (polygonColumnCount - 1) + k * 0.001) + " " +
                                    std::to_string(10.5 + 5 * (polygonColumnCount - 1) + k * 0.001) + ", " +
                                    std::to_string(20.5 + 5 * (polygonColumnCount - 1) + k * 0.001) + " " +
                                    std::to_string(10.5 + 5 * (polygonColumnCount - 1) + k * 0.001) + ", " +
                                    std::to_string(25 + 5 * (polygonColumnCount - 1) + k * 0.001) + " " +
                                    std::to_string(15.5 + 5 * (polygonColumnCount - 1) + k * 0.001) + ", " +
                                    std::to_string(20.5 + 5 * (polygonColumnCount - 1) + k * 0.001) + " " +
                                    std::to_string(20 + 5 * (polygonColumnCount - 1) + k * 0.001) + ", " +
                                    std::to_string(10.5 + 5 * (polygonColumnCount - 1) + k * 0.001) + " " +
                                    std::to_string(20 + 5 * (polygonColumnCount - 1) + k * 0.001) + ", " +
                                    std::to_string(5 + 5 * (polygonColumnCount - 1) + k * 0.001) + " " +
                                    std::to_string(15.5 + 5 * (polygonColumnCount - 1) + k * 0.001) + "))"));
            ...
            {
                stringColumnCount++;
                table.CreateColumn((std::string("colString") + std::to_string(stringColumnCount)).c_str(),
                                   COLUMN_STRING);
                auto& columns = table.GetColumns();
                auto& column = dynamic_cast<ColumnBase<std::string>&>(
                    *columns.at((std::string("colString") + std::to_string(stringColumnCount)).c_str()));
            ...
            case COLUMN_INT8_T: // used as a 8bit number but internally we use it as boolean
            {
                ...
                auto& columns = table.GetColumns();
                auto& column = dynamic_cast<ColumnBase<int8_t>&>(
                    *columns.at((std::string("colBool") + std::to_string(boolColumnCount)).c_str()));
            ...
            default:
            {
                ...
                for (int i = 0; i < blockCount; i++)
                {
                    ...
                    for (int k = 0; k < blockSize; k++)
                    {
                        ...
                        {
                            integerData.push_back(
                                sameDataInBlocks ? -1 : (k % (1024 * integerColumnCount)) * -1);
                        }
                    }
                    column.AddBlock(integerData);
                }
