...
GPUMemory::GPUPolygon ComplexPolygonFactory::PrepareGPUPolygon(const std::vector<ColmnarDB::Types::ComplexPolygon>& polygons, const std::string & columnName, size_t blockIndex)
{
	// Points of polygons
	std::vector<NativeGeoPoint> polyPoints;
	// Start indexes of each polygon in point array
...
ColmnarDB::Types::ComplexPolygon ComplexPolygonFactory::FromWkt(std::string wkt)
{
	...
	if (replaceIdx != std::string::npos)
	{
		polygons[polygons.size() - 1].erase(polygons[polygons.size() - 1].begin() + replaceIdx, polygons[polygons.size() - 1].end());
	}
	...
	for (const auto& polygon : polygons)
	{
		...
		for (const auto& point : points)
		{
			...
			if (coordCount != 2)
			{
				throw std::invalid_argument("Invalid WKT format - too many or not enough coordinates for a point");
			}
		...
		if (std::abs(newPolygon->geopoints(0).latitude() - newPolygon->geopoints(newPolygon->geopoints_size() - 1).latitude()) >= 0.0001f ||
			std::abs(newPolygon->geopoints(0).longitude() - newPolygon->geopoints(newPolygon->geopoints_size() - 1).longitude()) >= 0.0001f)
		{
