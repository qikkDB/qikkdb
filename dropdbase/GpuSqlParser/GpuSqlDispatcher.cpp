...
: database(database), blockIndex(dispatcherThreadId), instructionPointer(0), constPointCounter(0),
  ...
  groupByTables(groupByTables), dispatcherThreadId(dispatcherThreadId), insideGroupBy(false),
  usingGroupBy(false), usingOrderBy(false), usingJoin(false), isLastBlockOfDevice(false),
  isOverallLastBlock(false), noLoad(true), loadNecessary(1), cpuDispatcher(database),
  jmpInstuctionPosition(0), insertIntoData(std::make_unique<InsertIntoStruct>()),
  joinIndices(nullptr), orderByTable(nullptr), orderByBlocks(orderByBlocks)
{
}
                                       ...
                                       int8_t* nullMaskPtr)
{
    if (useCache)
    {
            ...
            Context::getInstance().getCacheForCurrentDevice().containsColumn(databaseName, colName + "_polyIdx", blockIndex))
        {
            ...
            polygon.pointIdx =
                std::get<0>(cache.getColumn<int32_t>(databaseName, colName + "_pointIdx", blockIndex, size));
            polygon.polyIdx =
