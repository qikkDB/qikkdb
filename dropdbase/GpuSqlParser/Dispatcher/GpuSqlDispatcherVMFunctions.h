...
int32_t GpuSqlDispatcher::retCol()
{
	...
	if (usingGroupBy)
	{
		...
		{
			PointerAllocation col = allocatedPointers.at(colName + (std::find_if(groupByColumns.begin(), groupByColumns.end(), StringDataTypeComp(colName)) != groupByColumns.end()? KEYS_SUFFIX : ""));
