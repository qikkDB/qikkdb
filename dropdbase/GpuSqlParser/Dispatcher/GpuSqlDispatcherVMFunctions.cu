...
std::array<GpuSqlDispatcher::DispatchFunction, DataType::DATA_TYPE_SIZE> GpuSqlDispatcher::retFunctions = {
...
{
    if (allocatedPointers.find(colName + "_polyPoints") == allocatedPointers.end() &&
        !colName.empty() && colName.front() != '$')
    {
        ...
        auto col = dynamic_cast<const ColumnBase<ColmnarDB::Types::ComplexPolygon>*>(
            database->GetTables().at(table).GetColumns().at(column).get());


        if (!usingJoin)
        {
            auto block =
                dynamic_cast<BlockBase<ColmnarDB::Types::ComplexPolygon>*>(col->GetBlocksList()[blockIndex]);
            ...
            if (block->GetNullBitmask())
            {
                ...
                {
                    nullMaskPtr =
                        reinterpret_cast<int8_t*>(allocatedPointers.at(colName + NULL_SUFFIX).gpuPtr);
                }
            }
        ...
        else
        {
            ...
            GPUJoin::reorderByJoinTableCPU<ColmnarDB::Types::ComplexPolygon>(joinedPolygons, outDataSize,
                                                                             *col, blockIndex,
                                                                             joinIndices->at(table),
                                                                             database->GetBlockSize());
                ...
                if (allocatedPointers.find(colName + NULL_SUFFIX) == allocatedPointers.end())
                {
                    ...
                    if (!std::get<2>(cacheMaskEntry))
                    {
                        ...
                        GPUJoin::reorderNullMaskByJoinTableCPU<ColmnarDB::Types::ComplexPolygon>(
                            std::get<0>(cacheMaskEntry), outMaskSize, *col, blockIndex,
                            joinIndices->at(table), database->GetBlockSize());
                    }
                }
                else
                {
    ...
    if (allocatedPointers.find(colName) == allocatedPointers.end() && !colName.empty() && colName.front() != '$')
    {
        ...
        if (!usingJoin)
        {
            ...
            auto cacheEntry = Context::getInstance().getCacheForCurrentDevice().getColumn<NativeGeoPoint>(
                database->GetName(), colName, blockIndex, nativePoints.size());
        ...
        else
        {
            ...
            GPUJoin::reorderByJoinTableCPU<ColmnarDB::Types::Point>(joinedPoints, outDataSize, *col,
                                                                    blockIndex, joinIndices->at(table),
                                                                    database->GetBlockSize());
...
{
    if (allocatedPointers.find(colName + "_allChars") == allocatedPointers.end() &&
        !colName.empty() && colName.front() != '$')
    {
        ...
        if (!usingJoin)
        {
            ...
            insertString(database->GetName(), colName,
        ...
        else
        {
            ...
            int32_t outDataSize;
            GPUJoin::reorderByJoinTableCPU<std::string>(joinedStrings, outDataSize, *col, blockIndex,
                                                        joinIndices->at(table), database->GetBlockSize());
    ...
    if (usingGroupBy)
    {
        if (isOverallLastBlock)
        {
            ...
            auto col = findStringColumn(
                colName + (std::find_if(groupByColumns.begin(), groupByColumns.end(),
                                        StringDataTypeComp(colName)) != groupByColumns.end() ?
                               KEYS_SUFFIX :
                               ""));
