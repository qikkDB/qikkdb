        ...
        !insideAggregation_)
    {
        if (isOverallLastBlock_)
        {
            ...
            if (std::get<2>(column))
            {
                ...
                int64_t* combinedMask = AllocateRegister<int64_t>(reg + KEYS_SUFFIX + NULL_SUFFIX, bitMaskSize);
                FillStringRegister(result, reg + KEYS_SUFFIX, retSize, true, combinedMask);
                GPUMemory::copyDeviceToDevice(combinedMask,
                                              reinterpret_cast<int64_t*>(std::get<2>(column)), bitMaskSize);
            }
            else
            {
    ...
    else if (isOverallLastBlock_ || !usingGroupBy_ || insideGroupBy_ || insideAggregation_)
    {
        ...
        if (!IsRegisterAllocated(reg))
        {
            ...
            if (std::get<2>(column))
            {
                ...
                int32_t bitMaskSize = NullValues::GetNullBitMaskSize(retSize);
                GPUMemory::copyDeviceToDevice(nullMask, reinterpret_cast<int64_t*>(std::get<2>(column)), bitMaskSize);
            }
            else
            {
            ...
            if (std::get<2>(columnLeft) || columnRight.GpuNullMaskPtr)
            {
                ...
                if (std::get<2>(columnLeft) && columnRight.GpuNullMaskPtr)
                {
                    GPUArithmetic::Arithmetic<ArithmeticOperations::bitwiseOr>(
                        combinedMask, reinterpret_cast<int64_t*>(std::get<2>(columnLeft)),
                        reinterpret_cast<int64_t*>(columnRight.GpuNullMaskPtr), bitMaskSize);
                }
                else if (std::get<2>(columnLeft))
                {
                    GPUMemory::copyDeviceToDevice(combinedMask,
                                                  reinterpret_cast<int64_t*>(std::get<2>(columnLeft)),
                                                  bitMaskSize);
                }
        ...
        !insideAggregation_)
    {
        if (isOverallLastBlock_)
        {
            ...
            if (column.GpuNullMaskPtr)
            {
                ...
                FillStringRegister(result, reg + KEYS_SUFFIX, retSize, true, combinedMask);
                GPUMemory::copyDeviceToDevice(combinedMask,
                                              reinterpret_cast<int64_t*>(column.GpuNullMaskPtr), bitMaskSize);
            }
            else
            {
                FillStringRegister(result, reg + KEYS_SUFFIX, retSize, true);
        ...
        if (isOverallLastBlock_)
        {
            ...
            if (std::get<2>(columnLeft) || std::get<2>(columnRight))
            {
                ...
                if (std::get<2>(columnLeft) && std::get<2>(columnRight))
                {
                    GPUArithmetic::Arithmetic<ArithmeticOperations::bitwiseOr>(
                        combinedMask, reinterpret_cast<int64_t*>(std::get<2>(columnLeft)),
                        reinterpret_cast<int64_t*>(std::get<2>(columnRight)), bitMaskSize);
                }
                else if (std::get<2>(columnLeft))
                {
                    GPUMemory::copyDeviceToDevice(combinedMask,
                ...
                {
                    GPUMemory::copyDeviceToDevice(combinedMask,
                                                  reinterpret_cast<int64_t*>(std::get<2>(columnRight)),
                                                  bitMaskSize);
                }
            }
            else
