...
GpuSqlDispatcher::InstructionStatus GpuSqlDispatcher::CastNumericToStringCol()
{
        ...
        !insideAggregation_)
    {
        if (isOverallLastBlock_)
        {
            ...
            {
                int32_t bitMaskSize = NullValues::GetNullBitMaskSize(retSize);
                int64_t* nullMask = AllocateRegister<int64_t>(reg + KEYS_SUFFIX + NULL_SUFFIX, bitMaskSize);
    ...
    else if (isOverallLastBlock_ || !usingGroupBy_ || insideGroupBy_ || insideAggregation_)
    {
        ...
        if (!IsRegisterAllocated(reg))
        {
            ...
            {
                int32_t bitMaskSize = NullValues::GetNullBitMaskSize(retSize);
                int64_t* nullMask = AllocateRegister<int64_t>(reg + NULL_SUFFIX, bitMaskSize);
