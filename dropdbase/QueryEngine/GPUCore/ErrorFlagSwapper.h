...
public:
	ErrorFlagSwapper() {
		GPUMemory::allocAndSet(&errorFlagPointer, static_cast<int32_t>(QueryEngineErrorType::GPU_EXTENSION_SUCCESS), 1);
	}
	...
	void Swap() {
		...
		GPUMemory::memset(errorFlagPointer, static_cast<int32_t>(QueryEngineErrorType::GPU_EXTENSION_SUCCESS), 1);

		if (errorFlag != QueryEngineErrorType::GPU_EXTENSION_SUCCESS)
		{
			QueryEngineError::setType(static_cast<QueryEngineErrorType>(errorFlag));
		}
		else
		{
