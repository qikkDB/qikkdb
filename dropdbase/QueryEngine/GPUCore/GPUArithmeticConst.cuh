...
/// <param name="errorFlag">flag for error checking</param>
template<typename T, typename U, typename V>
__global__ void kernel_minus_const_col(T *output, U AConst, V *BCol, int32_t dataElementCount, int32_t* errorFlag, T min, T max)
{
	int32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
	int32_t stride = blockDim.x * gridDim.x;
	...
	for (int32_t i = idx; i < dataElementCount; i += stride)
	{
		...
		{
			// Check for overflow
			if (((BCol[i] > V{ 0 }) && (AConst < (min + BCol[i]))) ||
				((BCol[i] < V{ 0 }) && (AConst > (max + BCol[i]))))
			{
				atomicExch(errorFlag, static_cast<int32_t>(QueryEngineError::GPU_INTEGER_OVERFLOW_ERROR));
				continue;
			}
		}
		output[i] = AConst - BCol[i];
	}
}
...
/// <summary>
/// Kernel FLOOR DIVISION (constant / column)
/// </summary>
/// <param name="output">output result data block</param>
/// <param name="AConst">left input operand</param>
/// <param name="BCol">block of the right input operands</param>
...
/// <param name="errorFlag">flag for error checking</param>
template<typename T, typename U, typename V>
__global__ void kernel_floor_division_const_col(T *output, U AConst, V *BCol, int32_t dataElementCount, int32_t* errorFlag)
{
	int32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
	int32_t stride = blockDim.x * gridDim.x;
...
/// <summary>
/// Kernel DIVISION (constant / column) - as T always use some kind of floating point type!
/// </summary>
/// <param name="output">output result data block</param>
/// <param name="AConst">left input operand</param>
/// <param name="BCol">block of the right input operands</param>
...
/// <param name="errorFlag">flag for error checking</param>
template<typename T, typename U, typename V>
__global__ void kernel_division_const_col(T *output, U AConst, V *BCol, int32_t dataElementCount, int32_t* errorFlag)
{
	int32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
	int32_t stride = blockDim.x * gridDim.x;
	...
	for (int32_t i = idx; i < dataElementCount; i += stride)
	{
		...
		if (!std::is_floating_point<U>::value && !std::is_floating_point<V>::value)
		{
			output[i] = AConst / static_cast<T>(BCol[i]); // convert divisor to type T (should be floating point)
		}
		else
		{
			output[i] = AConst / BCol[i];
...
/// <summary>
/// Kernel MODULO (constant / column) - as U and V never use floating point type!
/// </summary>
/// <param name="output">output result data block</param>
/// <param name="AConst">left input operand</param>
/// <param name="BCol">block of the right input operands</param>
...
/// <param name="errorFlag">flag for error checking</param>
template<typename T, typename U, typename V>
__global__ void kernel_modulo_const_col(T *output, U AConst, V *BCol, int32_t dataElementCount, int32_t* errorFlag)
{
	int32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
	int32_t stride = blockDim.x * gridDim.x;
...
public:
	...
	template<typename T, typename U, typename V>
	static void minus(T *output, U AConst, V* BCol, int32_t dataElementCount)
	{
		ErrorFlagSwapper errorFlagSwapper;

		kernel_minus_const_col <T, U, V>
			<< < Context::getInstance().calcGridDim(dataElementCount), Context::getInstance().getBlockDim() >> >
	...
	template<typename T, typename U, typename V>
	static void floorDivision(T *output, U AConst, V *BCol, int32_t dataElementCount)
	{
		ErrorFlagSwapper errorFlagSwapper;

		kernel_floor_division_const_col <T, U, V>
			<< < Context::getInstance().calcGridDim(dataElementCount), Context::getInstance().getBlockDim() >> >
			(output, AConst, BCol, dataElementCount, errorFlagSwapper.getFlagPointer());
	...
	template<typename T, typename U, typename V>
	static void division(T *output, U AConst, V *BCol, int32_t dataElementCount)
	{
		// TODO Uncomment when dispatcher is ready for this
		////result of this type of division operation is always floating point - so check type T
		//static_assert(std::is_floating_point<T>::value,
		...
		kernel_division_const_col <T, U, V>
			<< < Context::getInstance().calcGridDim(dataElementCount), Context::getInstance().getBlockDim() >> >
			(output, AConst, BCol, dataElementCount, errorFlagSwapper.getFlagPointer());
	...
	template<typename T, typename U, typename V>
	static void modulo(T *output, U AConst, V *BCol, int32_t dataElementCount)
	{
		//modulo is not defined for floating point type
		static_assert(!std::is_floating_point<U>::value && !std::is_floating_point<V>::value,
			"None of the input columns of operation modulo cannot be floating point type!");
