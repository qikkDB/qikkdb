...
public:
    ...
    template<typename T>
	static bool compressDataAAFL(T* const hostUncompressed, int64_t uncompressedElementsCount, std::vector<T>& hostCompressed, int64_t& compressedElementsCount, T minValue, T maxValue);
	...
	template<typename T>
	static bool decompressDataAAFL(T* const hostCompressed, int64_t compressedElementsCount, std::vector<T>& hostUncompressed, int64_t &uncompressedElementsCount, T minValue, T maxValue);
	...
	/// <returns>Value representing result of decompression</returns>
	template<typename T>
	static bool decompressDataAAFLOnDevice(T* const deviceCompressed, int64_t uncompressedElementsCount, int64_t compressedElementsCount, int64_t compressionBlocksCount, T* const deviceUncompressed, T minValue, T maxValue);
