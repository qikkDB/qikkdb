...
void TestGroupByString(std::vector<std::vector<std::string>> keys, std::vector<std::vector<int32_t>> values,
	std::unordered_map<std::string, int32_t> correctPairs)
{
	...
	{
		ASSERT_FALSE(correctPairs.find(resultKeys[i]) == correctPairs.end()) << " key \"" << resultKeys[i] << "\"";
...
{
	TestGroupByString<AggregationFunctions::sum>(
		{ { "Apple", "Abcd" }, { "XYZ", "Abcd" }, { "ZYX", "XYZ" }, { "Apple", "Apple" } },
		{ { 1, 1 }, { 1, 2 }, { 1, 2 }, { 2, 4 } },
		{
			{ "Apple", 7 },
			{ "Abcd", 3 },
