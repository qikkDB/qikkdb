...
TEST(DispatcherNullTests, LimitOffsetNoClausesNoFullBlockNullTest)
{
    ...
    for (int32_t i = 0; i < blockSize / 2; i++)
    {
        ...
        else
        {
            ...
            if (i > 4 && expectedResults1.size() < 5)
            {
                ...
                expectedResults3.push_back(ssPoint.str());
                expectedResults4.push_back(ssPolygon.str());
            }
        }
    }
    ...
    auto& payload3 = result->payloads().at("TestTable.Col2");
    auto& nullBitMask3 = result->nullbitmasks().at("TestTable.Col2");

    auto& payload4 = result->payloads().at("TestTable.Col2");
    auto& nullBitMask4 = result->nullbitmasks().at("TestTable.Col2");
